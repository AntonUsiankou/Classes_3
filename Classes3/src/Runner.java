    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (PURCHASES_NUMBER >= 0 && PURCHASES_NUMBER <= 10) {
                ...
                double sumInMonday = 0;
                WeekDay currentDay = null;

                double priceOfDayByOneItem = 0;
                for (Purchase purchase : purchases) {
                    averageCost += Double.valueOf(Purchase.toByn(purchase.getCost()));
                    if (purchase.getWeekDay() == WeekDay.MONDAY) sumInMonday += Double.valueOf(Purchase.toByn(purchase.getCost()));
